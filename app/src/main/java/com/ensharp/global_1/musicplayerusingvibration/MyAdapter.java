...
public class MyAdapter extends BaseAdapter {
    ...
    public MyAdapter(Activity activity, List<MusicVO> list) {
        this.list = list;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        byte[] albumBytes = getAlbumImage(activity, Integer.parseInt((list.get(position)).getAlbumId()), 170);
        Bitmap albumImage;
        if(albumBytes == null)
            albumImage = null;
        else
            albumImage = BitmapFactory.decodeByteArray(albumBytes, 0, albumBytes.length);
    ...
    public static byte[] getAlbumImage(Context context, int album_id, int MAX_IMAGE_SIZE) {
